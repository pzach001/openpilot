import numpy as np
from common.numpy_fast import interp #lines
...
class LongitudinalMpc(object):
  def __init__(self, mpc_id, live_longitudinal_mpc):
    ...
    self.new_lead = False
    self.v_rel = 0.0 #lines
    self.lastTR = 2 #lines
    self.last_cloudlog_t = 0.0
    self.v_rel = 10 #lines
    self.last_cloudlog_t = 0.0 #lines
    self.currentTR = 2 #lines
    self.mpc_frame = 0 #lines
  ...
  def update(self, CS, lead, v_cruise_setpoint):
    ...
    if lead is not None and lead.status:
      x_lead = max(0, lead.dRel - STOPPING_DISTANCE)  # increase stopping distance to car by X [m] #lines
    ...
    t = sec_since_boot()
    n_its = self.libmpc.run_mpc(self.cur_state, self.mpc_solution, self.a_lead_tau, a_lead, TR) #lines
